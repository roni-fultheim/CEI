...
# Load intermediate -------------------------------------------------------
performance_analysis = list.files(path = "Summary/EI_run_stats/", pattern = "EIRunStats.csv", full.names = T) %>%
...
allData %>%
  select(Sample, contains("EditingIndex"), 
         starts_with("NextBestMMAfterA2G"), starts_with("Signal2NoiseRatio"), 
         matches("NumOf[AT].*"), matches("NumOf[AT]2[GC]Mismatches.*"), -matches("NumOf[AT]2[GC]MismatchesSites.*"), starts_with("NormalizedCoveragePerSite_")) %>%
  mutate(TotalAGenomeWideAlu = site_count$GenomeWideAlu,
         TotalAIRAlu3UTR = site_count$IRAlu3UTR) %>% 
  left_join(performance_analysis %>%
               select(Sample, EI_type, `Total elapsed time (seconds)`, `Maximum resident set size (kbytes)`) %>%
               rename("Total_elapsed_time_seconds" = `Total elapsed time (seconds)`, 
                      "Maximum_resident_set_size_kbytes" = `Maximum resident set size (kbytes)`) %>%
               pivot_wider(names_from = EI_type, id_cols = Sample, values_from = c(Total_elapsed_time_seconds, Maximum_resident_set_size_kbytes))) %>%
  rename_with(.cols = everything(), .fn = ~str_replace(.x, pattern = "GenomeWideAlu", replacement = "Global")) %>%
  rename_with(.cols = everything(), .fn = ~str_replace(.x, pattern = "IRAlu3UTR", replacement = "Cytoplasmic")) %>%
  fwrite(file = file.path(out_stats, "InfectiousDiseasesData_Editing_Coverage_RunStats.csv"), quote = F, append = F, scipen = 999)
...
correlation_coef_fig4a = coef(lm(allData$A2GEditingIndexIRAlu3UTR ~ 0 + allData$A2GEditingIndexGenomeWideAlu))
fig4a = allData %>%
  ...
  geom_abline(slope = correlation_coef_fig4a, intercept = 0, color = "#8B1A1A") +
  ...
  # annotate slope
  annotate("text",x=2,y=0.5, size = 5, hjust = 0, label=(paste0("Slope==",signif(correlation_coef_fig4a, digits = 3))),parse=TRUE, color = chosen_colors[2])+ 
  # # annotate line
...
correlation_coef_fig4b = coef(lm(temp$Signal2NoiseRatio_IRAlu3UTR ~ 0 + temp$Signal2NoiseRatio_GenomeWideAlu))
fig4b = temp %>%
  ...
  annotate("text",x=45,y=375, size = 5, hjust = 0, label=(paste0("Slope==",signif(correlation_coef_fig4b, digits = 3))),parse=TRUE, color = chosen_colors[2])+
  # # annotate line
...
# Fig. 3C -----------------------------------------------------------
correlation_coef_fig4c = coef(lm(allData$NormalizedCoveragePerSite_IRAlu3UTR ~ 0 + allData$NormalizedCoveragePerSite_GenomeWideAlu))
fig4c = allData %>%
  ...
  theme(legend.text  = element_text(angle = 45, hjust = 1)) +
  # annotate slope
  annotate("text",x=3.7,y=52, size = 5, hjust = 0, label=(paste0("Slope==",signif(correlation_coef_fig4c, digits = 3))),parse=TRUE, color = chosen_colors[2])+
...
# Fig. 3D -----------------------------------------------------------------
fig4d = l_EI_wMETA_a2g_filtered %>%
  group_by(BioProject, `Editing Index Type`) %>%
  ...
  ggtitle("Normalized standard deviation", subtitle = "Per dataset") +
...
fwrite(l_EI_wMETA_a2g_filtered %>%
         group_by(BioProject, `Editing Index Type`) %>%
         summarise(across(c(EditingIndex), list(sd = sd,
                                                mean = mean)),
                   NumSamples = n()) %>%
         mutate("Normalized_standard_deviation" = EditingIndex_sd / EditingIndex_mean) %>%
         relocate(BioProject, `Editing Index Type`, contains("EditingIndex"), Normalized_standard_deviation, NumSamples), 
       file = file.path(out_stats, "Figure3D_NumberOfSamplesTable.csv"), quote = F, scipen = 999)

# Fig. 3E -----------------------------------------------------------------

fig4e = l_EI_wMETA_a2g %>% 
  mutate(`Editing Index Type` = forcats::fct_recode(`Editing Index Type`, "AEI" = "Global Index", 
                                                    "CEI" = "Cytoplasmic Index") %>% 
           forcats::fct_relevel("CEI")) %>%
...
# fig4 <- plot_grid(plot_grid(fig4a, fig4b,rel_widths = c(0.25, 0.75),
#                             labels=c("B", "C"), ncol = 2, nrow = 1, align = 'hv', label_size=18),
#                   plot_grid(fig4c, fig4e,
#                             labels=c("D", "E"), ncol = 2, nrow = 1, align = 'hv', label_size=18),
#                   labels=c("", ""), ncol = 1, nrow = 2, align = 'hv', label_size=18)
fig4 <- plot_grid(plot_grid(fig4a, fig4b, fig4c, fig4d, rel_widths = c(1, 1, 1, 1), 
                            labels=c("A", "B", "C", "D"), ncol = 4, nrow = 1, align = 'hv', label_size=18),
                  plot_grid(fig4e + 
                              # inset
                              patchwork::inset_element(fig4e_inset,
                                                       left   = 0.01, right = 0.3,
                                                       bottom = 0.43, top   = 0.98), 
                            fig4f,  fig4g, rel_widths = c(2.2, 0.65, 0.65),
...
save_plot(file.path(out_plots,"Figure4.pdf"), fig4, ncol = 1, nrow = 2, base_height = 8, base_width = 16)
save_plot(file.path(out_plots,"Figure4.png"), fig4, ncol = 1, nrow = 2, base_height = 8, base_width = 16)
